...
import { encodeWinAnsi, sanitizePdfText } from '../core/pdf-encoding.js';
...
class PageStream {
  ...
  text(xIn, line, topMarginIn, str, { bold, italic, underline } = {}) {
    ...
    this.ops.push(
      `BT /${font.key} ${FONT_SIZE} Tf 1 0 0 1 ${fmt(x)} ${fmt(y)} Tm (${esc(sanitizePdfText(str))}) Tj ET`
    );
    if (underline) {
...
export function exportPDF(doc, pagination, styles) {
  ...
  const addPage = (stream) => {
    ...
    const contentId = pdf.add(
      `<< /Length ${encodeWinAnsi(content).length} >>\nstream\n${content}\nendstream`
    );
    ...
    if (revHeader) {
      ...
      s.ops.push(
        `BT /F1 ${FONT_SIZE} Tf 1 0 0 1 ${fmt(styles.page.marginLeft * PT)} ${fmt(y)} Tm ` +
          `(${esc(sanitizePdfText(revHeader))}) Tj ET`
      );
    }
  ...
  const info = pdf.add(
    `<< /Title (${esc(sanitizePdfText(doc.title.title || 'Untitled'))}) ` +
      `/Author (${esc(sanitizePdfText(doc.title.author || ''))}) ` +
