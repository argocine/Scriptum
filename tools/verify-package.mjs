...
const ARTIFACT_EXTENSION = /\.(?:dmg|zip|exe|AppImage|deb)$/;

export function isTopLevelArtifact(file, distributionRoot) {
  return path.dirname(path.resolve(file)) === path.resolve(distributionRoot)
    && ARTIFACT_EXTENSION.test(file);
}
...
function verifyPackage() {
  ...
  walk(dist);
  const artifacts = files.filter((file) => isTopLevelArtifact(file, dist));
